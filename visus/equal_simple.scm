...
;translation x,y,z par rapport au sinus et au temps
            ...
            (push)
                ...
                (translate (vmul (vector n 0 0) (* 1.2 (* 10 (c "trans-coeff" id)))))
                (scale
                    (vector 
                        (* 10 (c "scale-x" id))
        ...
        (unless (zero? n)
            (equal_simple_sub (- n 1) id))
    )
)

(define (equal_simple id cross)
    (push)
        (translate
            (vector
                (* (- (* 2 (c "cam-x" id)) 1) 500)
