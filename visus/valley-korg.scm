...
(flxseed (inexact->exact (floor 1.2)))
    (letrec
        (
            (ncube (max 3 (c "ncube" id #:coeff 127)))
            (diametre (c "diametre" id #:coeff 10))
            (coeff-d-gh (c "coeff-d-gh" id #:coeff 10))
            (coeff-color (c "coeff-color" id #:coeff 100))
                        ...
                        ((and (zero? (quotient n (* 4 ncube))) (zero? (vector-ref (corner-select n) 0)))
                            (cond
                                ((zero? (vector-ref (corner-select n) 1))
                                    (pdata-set! "master" n (- diametre (* coeff-d-gh (gl (* n (/ 16 (flxrnd)))))))
                                    (pdata-set! "c" n (vmul (vector (gl n) (gl (* 2 n)) (gl (* 4 n)) 1) coeff-color))
                                    (pdata-get "master" n)
                                )
                                (else
